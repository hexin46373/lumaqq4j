...
public class MainShell implements IQQShowListener {        
	...
	private void onDiskViewerSelection(SelectionChangedEvent event) {
		...
		if(s == null || s.isEmpty()) {
			...
			setDiskOpHint(NLS.bind(disk_hint_capacity, FileTool.getSizeString(diskManager.getUnused()), FileTool.getSizeString(diskManager.getCapacity())));
			...
			if(obj instanceof File) {
				File file = (File)obj;
				setDiskOpHint(NLS.bind(disk_hint_file, FileTool.getSizeString(file.size), DateTool.format(file.modifiedTime)));
			} else
				setDiskOpHint(NLS.bind(disk_hint_capacity, FileTool.getSizeString(diskManager.getUnused()), FileTool.getSizeString(diskManager.getCapacity())));
			...
			tiUpload.setEnabled(obj instanceof Directory && diskManager.isChildCreatable((Directory)obj) || obj instanceof Integer && (Integer)obj == DiskContentProvider.MY_DISK);
		}
	}
