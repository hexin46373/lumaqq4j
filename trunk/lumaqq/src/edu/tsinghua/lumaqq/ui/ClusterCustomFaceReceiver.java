 ...
 * @author luma
 */
public class ClusterCustomFaceReceiver implements IQQListener {
    ...
    public void start() {     
        ...
        if(createPort()) {
	        packetCount = 0;
	        main.getClient().requestFace(im.header.sender, parser.getSessionId(currentIndex), parser.getAgentKey(), conn.getId());            
        } else
            clear();
    }
    ...
    public void qqEvent(QQEvent e) {
        switch(e.type) {
            ...
            case QQEvent.QQ_RECEIVE_FACE_INFO:
            	...
            	break;
            case QQEvent.QQ_RECEIVE_FACE_DATA:
    ...
    private void processReceiveFaceData(QQEvent e) {
        ...
        if(p.sessionId == parser.getSessionId(currentIndex)) {
            ...
            if(position >= buffer.length) {
                ...
                buffer = null;
                // 发送回复
                main.getClient().replyFaceData(p.sessionId, conn.getId());                
                // 继续得到其他表情
			    findNextFaceNeedToBeDownloaded();
			    if(currentIndex < count) {
	                // 得到中转服务器地址，开始请求
...
//    }
    ...
    private void processReceiveFaceInfo(QQEvent e) {
        ...
        if(packet.sessionId == parser.getSessionId(currentIndex)) {
            ...
            // 请求发送文件数据
	        main.getClient().requestData(packet.sessionId, conn.getId());            
        }
    }
