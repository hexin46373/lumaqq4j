 ...
 * @author luma
 */
public class GetCustomHeadInfoJob extends AbstractJob implements IQQListener {
	...
	protected void preLoop() {
		...
		page = 1;
		main.getClient().getCustomHeadInfo(friends.subList(0, Math.min(MAX_INFO, friends.size())), conn.getId());
	}
	
	public void qqEvent(QQEvent e) {
		switch(e.type) {
			case QQEvent.QQ_GET_CUSTOM_HEAD_INFO_SUCCESS:
	...
	private void postGetCustomHeadInfo() {
		if(friends.size() > page * MAX_INFO) {
			main.getClient().getCustomHeadInfo(friends.subList(page * MAX_INFO, Math.min(friends.size(), page * MAX_INFO + MAX_INFO)), conn.getId());
			page++;
		} else {
			wake();
		}
